...
OUT = "results/llama32/surprisal_decay"
os.makedirs(OUT, exist_ok=True)

STORY = "Story"
OUTCOME = "DeltaSurprisal"
DIST_TYPES = ["CD", "RD"]
...
distance_counts.to_csv(os.path.join(OUT, "surprisal_distance_counts.csv"), index=False,)
...
for dist in DIST_TYPES:
    ...
    with open(os.path.join(OUT, f"{dist}_surprisal_distance_ols.txt"), "w", encoding="utf-8") as file:
...
all_term_tests.to_csv(os.path.join(OUT, "surprisal_distance_ols_tests.csv"), index=False)
...
distance_effects.to_csv(os.path.join(OUT, "surprisal_distance_effects_ols.csv"), index=False)
boundary_by_distance.to_csv(os.path.join(OUT, "surprisal_boundary_effects_by_distance.csv"), index=False)
boundary_modulation.to_csv(os.path.join(OUT, "surprisal_boundary_modulation.csv"), index=False)
...
curve_comparison.to_csv(os.path.join(OUT, "surprisal_curve_model_comparison.csv"), index=False)
curve_parameters.to_csv(os.path.join(OUT, "surprisal_curve_parameters.csv"), index=False)
best_models.to_csv(os.path.join(OUT, "surprisal_best_curve_models.csv"), index=False)
...
with open(os.path.join(OUT, "direct_CD_RD_surprisal_ols.txt"), "w", encoding="utf-8") as file:
...
direct_wald.to_csv(os.path.join(OUT, "direct_CD_RD_surprisal_wald.csv"), index=False)
...
direct_persistence.to_csv(os.path.join(OUT, "direct_CD_RD_surprisal_persistence.csv"), index=False)
...
direct_boundary.to_csv(os.path.join(OUT, "direct_CD_RD_surprisal_boundary.csv"), index=False)
...
with plt.rc_context({
    ...
    for column, dist in enumerate(DIST_TYPES):
        ...
        ax.set_title(f"{dist} surprisal trajectory")
    ...
    axes[0, 0].set_ylabel("Distorted − Original\ncontextual surprisal")
    ...
    ax.set_ylabel("Additional surprisal effect\nat sentence boundary")
    ...
    ax.set_ylabel("Model-estimated surprisal difference")
    ...
    fig.savefig(os.path.join(OUT, "surprisal_distance_results.svg"), format="svg", bbox_inches="tight", facecolor="white")
    plt.show()
