...
for (story, version), story_data in tqdm(story_groups, total=story_groups.ngroups, desc="Qwen inference"):
...
llm_effects.to_csv(os.path.join(OUT, "qwen_word_effects.csv"), index=False)
...
position_effects.to_csv(os.path.join(OUT, "qwen_position_effects.csv"), index=False)
...
position_results.to_csv(os.path.join(OUT, "qwen_position_regression_results.csv"), index=False,)
...
fig.savefig(os.path.join(OUT, "qwen_position_effects_combined.svg"), format="svg", bbox_inches="tight")
plt.show()
